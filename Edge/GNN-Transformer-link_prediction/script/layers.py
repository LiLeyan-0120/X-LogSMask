...
class MultiHeadAttention(torch.nn.Module):
    ...
    def forward(self, values, keys, queries, graph=None):
        ...
        if graph is not None:
            ...
            # graph = zero_out_random_elements(graph, 100)  # da(cora:100 ) can boost one point
            ...
            # energy = torch.mul(energy, graph.unsqueeze(0).unsqueeze(0))  # v3: Can the method of replacing with extremely small values be given by a formula
